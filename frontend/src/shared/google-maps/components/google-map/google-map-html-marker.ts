import { Container, autoinject, view, useShadowDOM, bindable } from "aurelia-framework";
...
import { createHtmlOverlayView, geoJsonPointToLatLng, IHtmlOverlayView } from "./google-map-utilities";
...
export class GoogleMapHtmlMarkerCustomElement extends GoogleMapObject<IHtmlOverlayView>
{
     ...
     */
    public constructor(element: Element, map: GoogleMapCustomElement, container?: Container)
    {
        super(map);
    ...
    public attach(): void
    {
        this.instance = createHtmlOverlayView(
        {
            ...
            ready: () => this.content.style.display = ""
        });

        for (const eventName of eventNames)
        {
            this._eventManager.addEventListener(this.content, eventName, (event: Event) =>
            {
