...
/**
 * Custom element representing an input for picking a time zone.
 */
@autoinject
export class ZoneInputCustomElement
{
    /**
    ...
    protected get inputValue(): string
    {
        ...
        else
        {
            ...
            if (this.value != null)
            {
                return this.formatZone(this.value);
            }
        }
    ...
    /**
     * Formats the specified zone for presentation.
     * @param zone The zone to format.
     * @returns The specified zone, formatted for presentation.
     */
    protected formatZone(zone: IANAZone): string
    {
        ...
        const formattedOffset = `${offset > 0 ? "+" : ""}${Duration.fromObject({ minutes: offset }).toFormat("hh:mm")}`;

        return `(GMT ${formattedOffset}) ${zone.name}`;
    }
}
