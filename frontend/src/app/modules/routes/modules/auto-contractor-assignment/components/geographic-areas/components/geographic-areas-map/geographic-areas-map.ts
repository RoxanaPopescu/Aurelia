...
export class GeographicAreasMapCustomElement
{
     ...
     */
    @bindable
    public areas: AutoContractorAssignmentRule[] | undefined;

    /**
     * The function to call when an area is clicked.
     */
    @bindable
    public areaClick: CallbackWithContext<{ area: AutoContractorAssignmentRule }>;
    ...
    protected tryFitBounds(): void
    {
        ...
        if (this.areas != null)
        {
            ...
            {
                if (area.conditions.polygons != null)
                {
                    for (const polygon of area.conditions.polygons)
                    {
                        for (const p of polygon.coordinates[0])
                        {
