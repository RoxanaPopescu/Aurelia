...
export async function configure(aurelia: Aurelia): Promise<void> {
  ...
  LogManager.setLevel(
    ENVIRONMENT.debug ? LogManager.logLevel.debug : LogManager.logLevel.warn
  );
  ...
  aurelia.use.postTask(async () => {
    ...
    if (ENVIRONMENT.stubs) {
      const { stubs } = await import(
        /* webpackChunkName: "stubs" */ 'resources/stubs'
      );
      settings.infrastructure.api.interceptors.push(
        new ResponseStubInterceptor(stubs, 50)
      );
    }
    ...
    await Promise.all(
      [
        async () => {
          ...
          await import(
            /* webpackChunkName: "styles" */ 'resources/styles/index.scss' as any
          );
          await import(
            /* webpackChunkName: "theme-" */ `resources/themes/${themeService.theme.slug}/styles/index.scss`
          );
          await import(
            /* webpackChunkName: "theme-" */ `resources/themes/${themeService.theme.slug}/index.ts`
          );
        },
        async () => {
...
function setCurrency(
  ...
  oldCurrency: Currency | undefined,
  finish: () => void
): void {
